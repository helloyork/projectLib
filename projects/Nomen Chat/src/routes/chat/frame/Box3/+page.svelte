<script>
    ...
    let initMessageList = ["Hi there! 我是Nomen，有什么帮得上的忙吗？", "你好！Nomen随时为你解答API疑问"];
    let isFocused = false;
    let isSending = false;
    ...
    if (browser) {
      ...
      if (!window.NomenMain.ws) {
        ws = window.NomenMain.ws = new WebSocket("wss://radeo2.laf.dev/__websocket__");
        ws.onclose = function () {
        ...
        if (inputText.length && !NomenMain.wsSending && (isFocused || fc)) {
          ...
          if (!window.NomenMain.ws) {
            ...
            try {
              ...
              createErrorBubble(`抱歉，出现了一些意外错误，请联系Nomen解决，错误信息: ${err.message}`)
            }
          }
          inputText = "";
        }
